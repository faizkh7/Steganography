...
def embed_image_in_video(image_path, video_path, output_path, passkey):
    ...
    for frame_idx, x, y, bits in scattered_positions:
        ...
        #hidden_pixels.append((frame_idx, x, y, frames[frame_idx][y, x]))

    # Sort the list based on frame_idx
    #hidden_pixels.sort(key=lambda item: item[0])
...
embed_image_in_video('CSDF/Exp1/passenger.jpg', 'CSDF/Exp1/video.mp4', 'CSDF/Exp1/output_video.mp4', 'faizkhan')

# Extract the image from the video
img_height, img_width = 418, 612  # Hardcoded dimensions of the original image
extracted_image = extract_image_from_video('CSDF/Exp1/output_video.mp4', 'faizkhan', img_height, img_width)
